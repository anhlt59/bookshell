...
# https://github.com/uclouvain/openjpeg/archive/refs/tags/v2.3.1.tar.gz
RUN set -xe; \
...
# Install Poppler data (https://poppler.freedesktop.org/poppler-data-)

ENV VERSION_POPPLER_DATA=0.4.11
ENV POPPLER_DATA_BUILD_DIR=${BUILD_DIR}/poppler

RUN set -xe; \
    mkdir -p ${POPPLER_DATA_BUILD_DIR}/bin; \
    curl -Ls https://poppler.freedesktop.org/poppler-data-${VERSION_POPPLER_DATA}.tar.gz \
    | tar xzC ${POPPLER_DATA_BUILD_DIR} --strip-components=1
